...
region = "eu-west-2"
os.environ["ROLE_ARN"] = f"arn:aws:iam::{ACCOUNT_ID}:role/${ROLE_NAME}"
access_key = session.get_credentials().access_key
secret_key = session.get_credentials().secret_key
session_token = session.get_credentials().token
assumed_role_object = sts_client.assume_role(
    RoleArn=os.getenv("ROLE_ARN"),
    RoleSessionName="sigv4_role",
)
...
def get_canonical_uri_and_payload(query_type, query, method):
    ...
    if query_type == "sparql":
        ...
        print(
            'Third parameter should be from ["gremlin", "sparql", "sparqlupdate", "loader", "status] but is "'
            + query_type
            + '".'
        )
        sys.exit()
...
def make_signed_request(host, method, query_type, query):
    ...
    if method == "GET":
        ...
        r = requests.get(
            request_url, headers=request.headers, verify=True, params=params
        )
        ...
        # print("Request URL = " + request_url)
        r = requests.post(
            request_url, headers=request.headers, verify=True, data=data, timeout=5
        )
    ...
    if r:
        ...
        # if method == "POST":
        #     os.environ["loadid"] = str(j["payload"]["loadId"])
        #     # print(os.environ.get('loadid', 'Not Set'))
        # elif method == "GET":
        #     j = json.loads(response)
        #     os.environ["result"] = str(j["payload"]["overallStatus"]["status"])
        #     # print(result)
...
def parse_input_and_query_neptune():
    ...
    group_port.add_argument(
        "-p", "--port", type=int, help="port ex. 8182, default=8182", default=8182
    )
    group_action = parser.add_mutually_exclusive_group()
    group_action.add_argument(
        "-a", "--action", type=str, help="http action, default = GET", default="GET"
    )
    group_endpoint = parser.add_mutually_exclusive_group()
    group_endpoint.add_argument(
        "-q",
        "--query_type",
        type=str,
        help="query_type, default = status ",
        default="status",
    )
    group_data = parser.add_mutually_exclusive_group()
    group_data.add_argument(
        "-d", "--data", type=str, help="data required for the http action", default=""
    )
    ...
    if secret_key == "":
        print(
            "!!! ERROR: Your AWS_SECRET_ACCESS_KEY environment variable is undefined."
        )
        exit_and_print_help()
